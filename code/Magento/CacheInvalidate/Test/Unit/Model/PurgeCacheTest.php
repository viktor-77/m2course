...
class PurgeCacheTest extends TestCase
{
    ...
    public function testSendPurgeRequest(array $hosts): void
    {
        ...
        foreach ($uris as $uri) {
            $writeWithArgs[] = ['PURGE', $uri, '1.1', ['X-Magento-Tag-Pattern' => 'tags', 'Host' => $uri->getHost()]];
        }
    ...
    public function testSendMultiPurgeRequest(): void
    {
        ...
        $this->socketAdapterMock->expects($this->exactly(2))
            ...
            ->withConsecutive(
                [
                    'PURGE', $uri, '1.1',
                    ['X-Magento-Tag-Pattern' => implode('|', $tagsSplitA), 'Host' => $uri->getHost()]
                ],
                [
                    'PURGE', $uri, '1.1',
                    ['X-Magento-Tag-Pattern' => implode('|', $tagsSplitB), 'Host' => $uri->getHost()]
                ]
            );
